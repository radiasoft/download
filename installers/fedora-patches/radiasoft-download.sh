...
fedora_patches_mpich() {
    if grep -s -q format-security /usr/lib64/mpich/bin/mpifort; then
        ...
        install_sudo perl -pi -w -e 's{-Werror=format-security}{}' /usr/lib64/mpich/bin/mpifort
    fi
    if ! grep -s -q -- '-Wformat' /usr/lib64/mpich/bin/mpic{c,xx}; then
        install_sudo perl -pi -w -e 's{(?=-Werror=format-security)}{-Wformat }' /usr/lib64/mpich/bin/mpic{c,xx}
    fi
}
